...
def resize_image(input_image_path, output_image_path, size):
    ...
    print('The original image size is {wide} wide x {height} ' 'high'.format(
    ...
    print('The resized image size is {wide} wide x {height} ''high'.format(
        wide=width, height=height))
...
def index(request):
    ...
    usuario_info = {'foto_perfil': img_profile, 'fotos': media_count, 'seguidos': following,
                    'seguidores': follower, 'hastags': tags, 'username': username_insta}
    '''
            ...
            if user_u.user_insta is not None:
                if user_name is not False:
                    ...
                    if user is not None:
                        ...
                        messages.error(
                            request, 'Datos Incorrectos, Intente de Nuevo')
                ...
                messages.error(
                    request, "Debes Registrar tu cuenta de Instagram para poder ingresar.")
...
def registro(request):
    ...
    if request.method == "POST":
        ...
        if Usuarios.objects.filter(email=email).exists():
            ...
            u = Usuarios(nombre=nombre, apellido=apellido, email=email, password=password,
                         username=username, fin_registro=0, cambiar_pass_insta=0)
            u.save()
            context = {
                'username': username,
                'password': password
...
def registroinsta(request):
    ...
    if request.method == "POST":
        ...
        if api.login():
            Usuarios.objects.filter(username=username).update(
                user_insta=cuenta, pass_insta=password_i)
            ...
            # api.logout()
            messages.error(
                request, "Usuario no existe o Contraseñana invalida")
        ...
        if request.is_ajax():
            ...
            if (Productos.objects.filter(id_usuario_id=request.user.id, media_id=media_id).exists()):
                ...
                producto = Productos.objects.get(
                    id_usuario_id=request.user.id, media_id=media_id)
                ...
                Ventas(id_usuario=user_r, id_producto_id=producto.id, media_id=media_id, cantidad=cantidad, varias=varias, precio=precio,
                       nombre_cliente=nombre_cliente, telefono_cliente=telefono_cliente, descripcion_venta=descripcion_venta).save()
                print("producto.id: " + str(producto.id))
            else:
                print("sino")

                Productos(id_usuario=user_r, media_id=media_id, texto=texto,
                          descripcion_producto=descripcion_venta).save()
            ...
            for res in result_img:
                ...
                if not Imagenes.objects.filter(media_id=media_id, imagen=res.split(',')[1]).exists():
                    Imagenes(id_usuario=user_r, media_id=media_id,
                             imagen=res.split(',')[1]).save()
...
def ventas(request):
    ...
    following = api.LastJson['user']['following_count']
    tags = api.LastJson['user']['usertags_count']
    usuario = {'foto_perfil': img_profile, 'fotos': media_count, 'seguidos': following,
               'seguidores': follower, 'hastags': tags, 'username': username_insta}
    ...
    for v in todas_ventas:
        ...
        arr_ventas.append({'media_id': v.media_id, 'nombre_cliente': v.nombre_cliente, 'telefono_cliente': v.telefono_cliente, 'precio': v.precio, 'precio_total': (
            int(v.cantidad)*v.precio), 'descripcion_venta': v.descripcion_venta, 'cantidad': v.cantidad, 'imagen': arr_img, 'cant_img': len(arr_img)})
...
def nuevo_concurso(request):
    if (request.is_ajax()):
        ...
        if (media_info["items"][0]["media_type"] == 1):
            ...
            Concursos(id_usuario=user_r, img_url=img_url, media_id=media_id, hastags=hastag,
                      seguir_otros="", condiciones=comentario, view_otro=1).save()
        else:
            ...
            for img in media_info["items"][0]["carousel_media"]:
                ...
                Imagenes(id_usuario=user_r, media_id=media_id,
                         imagen=img["image_versions2"]["candidates"][0]['url']).save()
            ...
            Concursos(id_usuario=user_r, img_url="", media_id=media_id, hastags=hastag,
                      seguir_otros="", condiciones=comentario, view_otro=1).save()
        ...
        if Concursos.objects.filter(id_usuario_id=request.user.id, view_otro=1).exists():
            concursos = Concursos.objects.filter(
                id_usuario_id=request.user.id, view_otro=1)
            for c in concursos:
                imagenes = Imagenes.objects.filter(
                    id_usuario_id=request.user.id, media_id=c.media_id)
...
def publicar_concurso(request):
    if request.method == "POST":
        ...
        if request.is_ajax():
            ...
            if (media_id != 0):
                print("media_id != 0")
                concurso = Concursos.objects.filter(
                    id_usuario_id=request.user.id, media_id=media_id)
                for con in concurso:
...
def mis_concursos(request):
    ...
    for c in todos_concursos_2:
        ...
        if (imagen2):
            ...
            comentarios.append({'imagen2': imagen2, 'ruta_img': c.ruta_img, 'condiciones': c.condiciones, 'media_id': c.media_id, 'cont_comentario': cont_comentario,
                                'cont_validos': cont_validos, 'validos': validos, 'activo': c.activo, 'cont_likers': cont_likers, 'publi_ganadores': c.publi_ganadores})
        else:
            comentarios.append({'imagen2': None, 'ruta_img': c.ruta_img, 'condiciones': c.condiciones, 'media_id': c.media_id, 'cont_comentario': cont_comentario,
                                'cont_validos': cont_validos, 'validos': validos, 'activo': c.activo, 'cont_likers': cont_likers, 'publi_ganadores': c.publi_ganadores})
        print("comentarios["+str(i)+"]['imagen2']" +
              str(comentarios[i]['imagen2']))
        i += 1
...
def generar_ganadores(request):
    ...
    if request.is_ajax():
        ...
        if (despues == '0'):
            ...
            todos_concursos = Concursos.objects.filter(
                id_usuario_id=request.user.id, media_id=media_id)
            comentarios = []
            res_username = ""
            res_text = ""
                    ...
                    while no_gana:
                        for r in res:
                            if (r['username'] != res_username):
                                ...
                                res.append(
                                    {'username': res_username, 'text': res_text})
                                no_gana = False
                            else:
        ...
        if request.is_ajax():
            ...
            for c in concurso:
                ...
                for x in range(0, len(result_ganadores)):
                    Ganadores(id_usuario_id=request.user.id, id_concurso=c.id,
                              media_id=media_id, username=result_ganadores[x]).save()
                    pass
                c.activo = 0
                c.save()
...
def editar_producto(request):
    ...
    if request.is_ajax():
        ...
        if (Productos.objects.filter(id_usuario_id=request.user.id, media_id=media_id).exists()):
            ...
            Productos(id_usuario_id=request.user.id, media_id=media_id, talla=talla, modelo=modelo, cantidad=cantidad,
                      texto=texto, descripcion_producto=descripcion, res_automatica=res_automatica).save()
            user_r = Usuarios.objects.get(email=request.user.email)
...
def productos(request):
    ...
    for pro in product:
        ...
        products.append({'media_id': pro.media_id, 'texto': pro.texto, 'descripcion': pro.descripcion_producto, 'talla': pro.talla, 'modelo': pro.modelo,
                         'cantidad': pro.cantidad, 'disponible': pro.disponible, 'res_automatica': pro.res_automatica, 'imagen': arr_img, 'cant_img': len(arr_img)})
        arr_img = []
